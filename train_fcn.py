...
from fcn import FCN8s, FocalLoss, train_one_epoch, evaluate
...
def main(cfg):
    ...
    if os.path.exists(f"./saved/fcn_checkpoint.pth"):
        history = load_file(f"./saved/fcn_history.pkl")
        checkpoint = torch.load(f"./saved/fcn_checkpoint.pth", map_location=DEVICE)
    ...
    for epoch in range(cfg["epochs"]):
        ...
        }, f"./saved/fcn_checkpoint.pth")
        save_file(history, f"./saved/fcn_history.pkl")

    torch.save(model.state_dict(), f"./saved/fcn_e_{epoch+1}.pth")
